(function() {
	...
	var filterInput = document.getElementById('search-title');
    var filters = {};
    var checkboxValue;
    ...
    function reviewCallback(data) {
		...
		for (var i = 0; i < reviews.length; i++) {
			titles.push(reviews[i].review.seriesName);
		}
		filterInput.addEventListener('keypress', function(){
			for (var i = 0; i < titles.length; i++) {
				// console.log(this.value);
				console.log('Val:', this.value);
				console.log('Name:', this.name);
				console.log(titles[i]);
				if (titles[i].indexOf(this.value) !== -1 ){
				...
				} else if (titles[i].indexOf(this.value) == -1){
					checkboxValue = this.value;
